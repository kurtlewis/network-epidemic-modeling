...
public class SIRGraph extends AbstractEpidemicGraph {
    private double beta;
    private double mu;
    private Random rGen = new Random();

    public SIRGraph(String generator, int numNodes, double beta, double mu) {
    ...
    public void step() {
        ...
        for (Node node : infected) {
            ...
            while (neighborsIter.hasNext()) {
                ...
                // this node can only be infected if its not immune
                if (!immune.contains(neighbor) && this.rGen.nextDouble() < this.beta) {
